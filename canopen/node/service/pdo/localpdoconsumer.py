...
class LocalPDOConsumer(SYNCConsumer):
	""" LocalPDOConsumer
	...
	def attach(self, node, cob_id_rx = None, cob_id_sync = None):
		""" Attaches the ``LocalPDOConsumer`` to a ``Node``. It does NOT add or assign this ``LocalPDOConsumer`` to the ``Node``.
	...
	def detach(self):
		""" Detaches the ``LocalPDOConsumer`` from the ``Node``. It does NOT remove or delete the ``LocalPDOConsumer`` from the ``Node``. """
